...
export class FormComponent implements OnInit {
  ...
  onFileChange(event: any): void {
    ...
    if (file) {
      ...
      // You can set a preview URL if needed, or keep it for debugging
      this.file = file.name;
    } else {
  ...
  onSubmit(): void {
    if (this.myForm.valid && this.selectedFile) {
      ...
      formData.append('paper_type', this.myForm.get('paper_type')?.value); // Correct field name
      ...
      this.pastpaperService.uploadPastpaper(formData).subscribe(
        response => {
          ...
          this.showSuccessMessage = true; // Indicate success

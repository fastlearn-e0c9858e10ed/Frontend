  ...
  onSubmit() {
    if (this.myForm.valid && this.selectedFile) {
      ...
      this.pastpaperService.uploadPastpaper(formData).subscribe({
        ...
        error: (error) => {  console.error('Upload failed:', error);  if (error.error && error.error.detail) {    // Log the detailed error response from the server    console.error('Validation errors:', error.error.detail);    this.errorMessage = 'Upload failed: ' + error.error.detail.map((err: any) => err.message).join(', ');  } else {    this.errorMessage = 'Upload failed. Please check the form and try again.';  }}
      });
    } else {

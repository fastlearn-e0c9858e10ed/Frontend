...
export class FormComponent implements OnInit {
  ...
  onSubmit() {
    if (this.myForm.valid && this.selectedFile) {
      ...
      // Append the file
      formData.append('url', this.selectedFile, this.selectedFile.name); // Ensure 'url' is correct
      ...
      this.pastpaperService.uploadPastpaper(formData).subscribe({
        next: (response) => {
          ...
          console.log('Upload successful:', response); // Log the response for debugging
